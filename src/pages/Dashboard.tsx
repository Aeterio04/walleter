...
export default function Dashboard() {
  ...
  const { transactions, totalCredit, totalDebit, balance, totalInvested, categorySpending, budgets } = useApp()
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="w-[62.5%] border-r border-muted/30 flex flex-col">
        ...
        {overBudgetCategories.length > 0 && (
          <div
            ...
            className="border-b border-muted/30 p-6 bg-primary/5 cursor-pointer group"
          >
        ...
        <div className="p-8 border-b border-muted/30">
          <p className="text-[10px] text-muted uppercase tracking-[0.2em] mb-2">
            CURRENT BALANCE
          </p>
          <h2 className="font-display text-[72px] text-primary leading-none tracking-tight mono-number">
            {formatINR(balance)}
          </h2>
        ...
        <div className="flex-1 flex flex-col">
          <div className="px-8 py-4 border-b border-muted/30 flex justify-between items-center">
            <p className="text-[10px] text-muted uppercase tracking-[0.2em]">RECENT ENTRIES</p>
            <button
          ...
          <div className="grid grid-cols-[70px_1fr_100px_90px] px-8 py-3 text-[10px] text-muted uppercase tracking-[0.15em] border-b border-muted/20">
            <span>DATE</span>
          ...
          <div className="flex-1 overflow-auto">
            {recentTransactions.map((entry, i) => (
              <div
                key={entry.id}
                className="grid grid-cols-[70px_1fr_100px_90px] px-8 py-3 border-b border-muted/10 ledger-row cursor-default"
                onMouseEnter={() => setHoveredRow(i)}
                onMouseLeave={() => setHoveredRow(null)}
                style={{ backgroundColor: hoveredRow === i ? '#18181B' : 'transparent' }}
              >
      ...
      <div className="w-[37.5%] flex flex-col">
        ...
        <div className="p-6 border-b border-muted/30">
          ...
          <div className="border border-muted/20 p-4">
            <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-[180px]" preserveAspectRatio="none">
              ...
              <polyline points={incomePoints} fill="none" stroke="#CCFF00" strokeWidth="1.5" strokeLinejoin="bevel" />
              <polyline points={expensePoints} fill="none" stroke="#FA114F" strokeWidth="1" strokeLinejoin="bevel" strokeDasharray="4,2" />
            </svg>
        ...
        {/* Quick Actions */}
        <div className="p-6 border-b border-muted/30">
          <p className="text-[10px] text-muted uppercase tracking-[0.2em] mb-4">QUICK ACTIONS</p>
          <div className="grid grid-cols-2 gap-3">
            <button className="h-12 border border-primary text-primary text-xs uppercase tracking-[0.1em] font-bold btn-brutal bg-transparent" onClick={() => navigate('/expenses')}>
              ADD EXPENSE
            </button>
            <button className="h-12 border border-primary text-primary text-xs uppercase tracking-[0.1em] font-bold btn-brutal bg-transparent" onClick={() => navigate('/budget')}>
              SET BUDGET
            </button>
            <button className="h-12 border border-muted/50 text-muted text-xs uppercase tracking-[0.1em] font-bold btn-brutal bg-transparent" onClick={() => navigate('/investments')}>
              INVESTMENTS
            </button>
            <button className="h-12 border border-muted/50 text-muted text-xs uppercase tracking-[0.1em] font-bold btn-brutal bg-transparent" onClick={() => navigate('/insights')}>
              AI INSIGHTS
            </button>
          </div>
        </div>
        ...
        <div className="p-6 border-b border-muted/30 flex-1">
          <p className="text-[10px] text-muted uppercase tracking-[0.2em] mb-4">CATEGORY BREAKDOWN</p>
                ...
                <div key={cat.id}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className={`text-[10px] uppercase tracking-[0.15em] mono-number ${
                      pct > 100 ? 'text-danger' : pct > 80 ? 'text-primary' : 'text-muted'
                    }`}>
                  ...
                  <div className="h-2 bg-surface w-full">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: pct > 100 ? '#FA114F' : pct > 80 ? '#CCFF00' : '#71717A',
        ...
        {/* Portfolio summary */}
        <div className="p-6">
          <p className="text-[10px] text-muted uppercase tracking-[0.2em] mb-3">INVESTMENT STATUS</p>
